...
# ------------------------------------------------------------
#  Execution Engine: Hierarchical Clustering
# ------------------------------------------------------------
...
def heatmap(x, row_header, column_header, row_method,
            ...
            color_gradient, identifier, filename):
    ...
    if '/' in filename:
        ...
        dataset_name = string.split(filename,'\\')[-1]
        root_dir = string.join(string.split(filename,'\\')[:-1],'\\')+'\\'
    filename = root_dir+'%s-%s.clustered' % (identifier, dataset_name)
...
if __name__ == '__main__':
    ...
    if len(sys.argv[1:])<=1:  ### Indicates that there are insufficient number of command-line arguments
        ...
        options, remainder = getopt.getopt(sys.argv[1:],'', ['identifier=','i=','row_method=',
                                                    'row_metric='])
        for opt, arg in options:
            if opt == '--identifier': identifier=arg
            elif opt == '--i': filename=arg
    ...
    matrix, column_header, row_header = importData('/Applications/XAMPP/htdocs/bioinformatics/data-store/' + filename)

    if len(matrix)>0:
        try:
            heatmap(matrix, row_header, column_header, row_method, column_method, row_metric, column_metric, color_gradient, identifier, '/Applications/XAMPP/htdocs/bioinformatics/data-store/' + filename)
        except Exception:
